...
/** イベントの region（都市名）→ 地方エリア（関東/関西/中部…）への対応表。
 * イベント一覧のエリア絞り込みに使う。未登録の都市は areaOf() で「その他」。 */
...
/** エリアチップの並び順（該当イベントがあるものだけ、この順で表示する）。 */
export const AREA_ORDER = ["北海道", "東北", "関東", "中部", "関西", "中国・四国", "九州", "その他"];

/** イベントの region 文字列を地方エリアに変換する。未登録は「その他」。 */
export function areaOf(region: string): string {
  return REGION_TO_AREA[region] ?? "その他";
}
