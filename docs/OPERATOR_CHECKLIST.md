...
>
> **イベントの開催前リマインド（マイグレーション 0070）**：`create_event_reminders(days_ahead)`
> 関数を用意済みです。参加表明したイベントの開催前日に「まもなく開催」通知を作ります
> （冪等・二重送信なし）。自動化するには Supabase で pg_cron を有効化し、SQL Editor で
> 次を一度だけ実行してください（毎日 09:00 UTC に翌日開催分を送る例）：
>
> ```sql
> select cron.schedule('event-reminders-daily', '0 9 * * *',
>   $$ select create_event_reminders(1); $$);
> ```
>
> 登録せずに手動で試すには `select create_event_reminders(1);`。未登録でも通知が
> 作られないだけで、他機能への影響はありません。
>
> **開催後のレビュー催促（マイグレーション 0071）**：`create_review_nudges(days_after)`
> 関数を用意済みです。開催後、参加表明したのにまだレビューを書いていない人へ
> 「レビューしませんか？」通知を作ります（冪等・二重送信なし）。開催の2日後に催促する
> 日次ジョブの例：
>
> ```sql
> select cron.schedule('event-review-nudges-daily', '0 10 * * *',
>   $$ select create_review_nudges(2); $$);
> ```
>
> 手動テストは `select create_review_nudges(2);`。0070（開催前リマインド）とセットで
> 登録すると、集客→参加→レビュー蓄積のループが自動で回ります。
...
## ☐ ステップ 2ap: @ユーザーネーム/人物検索の土台（マイグレーション 0054）

`profile_username_search` が `false` のとき実行します。

1. リポジトリの **`supabase/migrations/0054_profile_username_search.sql`** を開く
2. 中身を**全部コピー**して SQL Editor に貼り付け、実行

→ `profiles` に `searchable_by_name` 列が追加されます。ユーザーがマイページの
「プロフィールを編集」から自分の **@ユーザーネーム（一意ID）** を設定でき、
プロフィールに `@xxxx` が表示されるようになります（handle 列自体は 0001 で既に
存在。人物検索は別途フォローの機能で追加予定）。`searchable_by_name` は「表示名
でも検索されてよいか」のフラグ（既定オフ・つきまとい対策）。`add column if not
exists` で冪等（再実行安全）。書き込みは本人のみ（既存の profiles_update RLS）。

---

## ☐ ステップ 2aq: @ユーザーネーム変更のなりすまし・乱用対策（マイグレーション 0055）

`handle_change_guard` が `false` のとき実行します。

1. リポジトリの **`supabase/migrations/0055_handle_change_guard.sql`** を開く
2. 中身を**全部コピー**して SQL Editor に貼り付け、実行

→ @ユーザーネームの変更にDBトリガーで2つの制限がかかります。
①admin/official/staff/premier など運営・公式を騙る文字列を含むユーザーネームへの
