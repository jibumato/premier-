...
ファン ──(¥1,000 支援)──▶ Stripe Checkout / PaymentIntent
                              │  application_fee_amount = ¥200（運営手数料 20%）
                              │  transfer_data.destination = 創作者の connected account
                              ▼
                         Stripe が分配
                              ├─▶ 運営（プラットフォーム）: ¥200 − Stripe決済手数料（約¥36）≒ ¥164
                              └─▶ 創作者の残高: ¥800 →（定期）銀行口座へ自動出金
```
...
- **手数料**: `application_fee_amount` で運営取り分を指定。**20% で確定**（§6）。
...
運営手数料は **20%**（クリエイター取り分 80%）とする。OnlyFans と同基準。

同界隈の相場（2026 時点・調査値。プラットフォーム取り分）：

| サービス | 全年齢 | R-18 | 形態 |
| --- | --- | --- | --- |
| OFUSE | 5%＋30円 | — | 投げ銭（単発） |
| Skeb | 9.8%（条件付 6.8%） | 同左 | リクエスト |
| pixivFANBOX | 10% | 12.9% | 月額支援 |
| Ci-en | 10% | 13% | 月額支援 |
| Fantia | 12.5%（非実写） | 17.5%（実写） | 月額・単発 |
| Patreon | 5〜12%（プラン別） | — | 月額支援 |
| **OnlyFans** | **20%** | **20%** | サブスク/チップ/PPV |
| **プルミエ！（本件）** | **20%** | **20%** | 投げ銭（単発） |

- **留意**: 単発の投げ銭市場としては 20% は高め（OFUSE 5% / Skeb 9.8% / FANBOX 10%）。
  新規サービスでは料率で比較されやすい。**料率は下げる方が上げるより容易**なので、20% で
  開始し、獲得状況を見て調整する前提。
- **実質粗利**: Stripe 決済手数料（約 3.6%）は運営手数料から支払う。20% 設定でも手元に
  残るのは概ね **16%強**（例：¥1,000 支援なら運営 ¥200 − Stripe ¥36 ≒ ¥164）。
- **少額対策（将来）**: ¥100 等の少額を許す場合、OFUSE の「率＋固定額（+30円）」型を
  検討（固定決済コスト負けの回避）。MVP は最低支援額を ¥300〜500 に設定して回避してもよい。
...
2. ~~**手数料率**~~ → **20% で確定**（OnlyFans 同基準。§5.5）。Stripe 決済手数料（約 3.6%）は
   この 20% から支払う（実質粗利 ≒ 16%強）。
