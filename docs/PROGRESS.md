...
- [x] **ローンチ準備（STEP 3 の一部）**: 空データ時のUX（`EmptyState`）をホーム/検索/メッセージ/
      おしらせ/知恵袋に追加。全20画面のプロトタイプ通しスモークをクリア（エラー0）。
      進行は [LAUNCH_PLAN.md](LAUNCH_PLAN.md) を参照
- [x] **ローンチ準備（STEP 1 の一部）**: 事業者情報（運営者=Type&Co・問い合わせ=16typeandco@gmail.com・
      利用年齢=18歳以上）を確定し規約/プライバシー/アプリ内に反映。越境移転（Supabase東京・
      Cloudflareエッジ）の記載も確定。残は弁護士レビュー
- [x] **運用マニュアル（STEP 2）**: [OPERATIONS.md](OPERATIONS.md) を作成。本人確認レビュー・
      通報対応（自動非表示＋重大案件の即対応）・問い合わせ・エスカレーションの運用ルールを明文化
- [x] **クローズドβ計画（STEP 4）**: [BETA_PLAN.md](BETA_PLAN.md) を作成。目的・規模・テスター配布文・
      確認チェックリスト・公開判断基準（Exit条件）を整備
- [x] **弁護士レビュー依頼メモ（STEP 1）**: [LEGAL_REVIEW_REQUEST.md](LEGAL_REVIEW_REQUEST.md) を作成。
      越境移転・本人確認書類・免責範囲・コスプレ特有の権利条項など、確認してほしい論点を優先度付きで整理
- [x] **弁護士レビューの反映（STEP 1・完了）**: 回答を受け、規約 v0.4 / プライバシー v0.3 に反映。
      免責に故意・重過失の除外を追加、年齢に18歳未満判明時のアカウント削除、管轄を地裁/簡裁に精緻化、
      越境移転に相当措置の確認方法を明記。アプリ内(TermsContent/PrivacyContent)にも反映。
      回答と対応の記録は [LEGAL_REVIEW_RESULT.md](LEGAL_REVIEW_RESULT.md)
