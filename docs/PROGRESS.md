...
| 知恵袋（Phase 4 の一部） | ✅ 100% |
| イベントカレンダー（Phase 4 の一部） | ✅ 100% |

- 実装済み画面: **24**（プロトタイプ 8 ・ 新規 12 ・ デザイン集 4）
- 直近の成果: **イベントカレンダーを実データ接続**（`events`/`event_rsvps` テーブル新設、モックの4イベントをシード。一覧・詳細・「参加表明する」(RSVP)を実データ化、参加予定人数は実カウント。イベントはキュレーション前提のためユーザーからの新規作成UIはなし。未接続時は元のプロトタイプ動作を完全維持）
- フェーズ進捗: **Phase 0・Phase 1 (P0)・Phase 2 (P1) 完了、Phase 3 (P2) はレビュー・他ユーザー閲覧完了、Phase 4 (P3) は知恵袋・イベント完了 → フリマ・eKYC/ゾーニングが残る**
...
| **Phase 4 (P3)** | コミュニティ拡張(フリマ/イベント/知恵袋) | market*/events*/qa* | 🟡 一部完了（知恵袋・イベント✅ / フリマ未着手） | 2–3週 | Phase 1–2 |
