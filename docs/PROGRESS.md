...
- 直近の成果: **フリマを実データ接続 → Phase 4 完了**（`market_items` テーブル新設。一覧・詳細を実データ化、「出品する」からインライン出品フォーム（画像アップロード付き）で実際に出品可能に。出品者への「メッセージ」は実会話を作成、出品者カードは実プロフィールへ遷移。出品者本人は「売却済みにする」でSOLD切替可（RLSで担保）。アプリ内決済はワンオペ制約によりスコープ外。未接続時は元のプロトタイプ動作を完全維持）
- フェーズ進捗: **Phase 0〜2 完了、Phase 3 (P2) はレビュー・他ユーザー閲覧完了（eKYC/ゾーニングのみ残）、Phase 4 (P3) 完了 → 残るは Phase 5 と eKYC**
...
- [ ] **残タスク**: Phase 5（通報自動処理・自動バッジ・法人掲載）← 次はここ
