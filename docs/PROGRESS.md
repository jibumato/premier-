...
- 直近の成果: **Phase 5 完了**（通報・ブロックを実データ化＋通報3人闾値でコンテンツを自動非表示にするDBトリガー。ホーム/検索フィードはブロック済みホスト・自動非表示併せを除外。併せ実績が闾値到達で「併せマイスター」バッジをトリガーで自動付与。法人掲載は実リード送信フォームに。全て人手審査なしの自動運営方針に準拠。未接続時はプロトタイプ動作を完全維持）
- フェーズ進捗: **Phase 0〜2 完了、Phase 4・Phase 5 完了、Phase 3 (P2) はレビュー・他ユーザー閲覧完了 → 残るは eKYC/ゾーニングのみ（手動本人確認で先行対応を検討中）**
...
- [ ] **Phase 3（本人確認）**: eKYC ベンダーは高コストのため、初期は**手動本人確認フォーム**
      （身分証アップロード→運営が目視確認→`is_verified`/`is_age_verified` を手動更新、画像は確認後即削除）で
      先行対応する方針を検討中。ゾーニング（年齢確認済みのみ支援リンク表示）は下地あり
- [ ] NSFW 判定 API の比較検討（画像投稿全般に適用）
- [ ] 通報の自動非表示フィルタを market/qa/messages にも展開（現状は awase フィードのみ）
