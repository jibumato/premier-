...
| 主催者の運営効率化（第1〜3弾） | ✅ 100% |
| コンテンツ拡充（お知らせ・イベント実データ・作品カタログ） | ✅ 100% |

- 実装済み画面: **26**（プロトタイプ 8 ・ 新規 14 ・ デザイン集 4）
- 直近の成果: **主催者の運営効率化（第1〜3弾）完了**。第1弾=応募者管理画面（承認/見送り・プロフ/メッセージ導線）＋定員到達で自動締切（トリガー）＋募集の複製。第2弾=公開予約(`publish_at`)・応募締切(`application_deadline`)・募集テンプレ(`awase_templates`)。第3弾=承認者への一斉連絡（DM一括送信＋リマインダー定型文）・キャンセル待ち受付(`accept_waitlist`)。マイグレーション 0017〜0019。
- フェーズ進捗: **全フェーズの設計スコープを実データ化完了**（Phase 0〜5＋本人確認手動版）＋**主催者運営ツール（第1〜3弾）**。残りは非機能・拡張（NSFW自動判定・リマインダー自動送信のpg_cron化・将来の eKYC/金銭仲介）のみ
...
| 運営効率化 | **第1弾**: 応募者管理画面（承認/見送り・繰上げ）＋定員自動締切トリガー＋募集の複製 | ✅ 完了 | Phase 1–2 | 済 |
| 運営効率化 | **第2弾**: 公開予約・応募締切・募集テンプレ保存（`awase_templates`） | ✅ 完了 | Phase 1 | 済 |
| 運営効率化 | **第3弾**: 承認者への一斉連絡（DM一括＋リマインダー）・キャンセル待ち受付 | ✅ 完了 | Phase 1–2 | 済 |
| 運営効率化 | リマインダーの自動送信（pg_cron/Edge Function） | ⏸ 保留 | 運用判断 | 未定 |
| コンテンツ | お知らせ機能（`announcements`・お知らせ画面・ウェルカムバナー） | ✅ 完了 | — | 済 |
| コンテンツ | イベント2026/2027の実データ化・作品カタログ拡充（約50作品）＋作品カバー | ✅ 完了 | Phase 4 | 済 |
...
- [x] **コンテンツ拡充**: お知らせ機能（`announcements`＋お知らせ画面＋ウェルカムバナー、[ANNOUNCEMENTS.md](ANNOUNCEMENTS.md)）、
      イベント2026/2027の実在データ化、作品カタログ拡充（近年人気の約50作品）＋作品カバー（タイポ＋固定カラー）、
      タグライン「“好き”で、つながる。」。マイグレーション 0012〜0016
- [x] **主催者の運営効率化 第1弾**（マイグレーション0017）: 応募者管理画面（`HostApplicantsScreen`／承認・見送り・
      応募中に戻す・プロフィール/メッセージ導線）、定員到達で募集を自動締切するトリガー、募集の複製（既存併せから
      作成フォームをプリフィル）。募集詳細のホスト操作に「応募者を見る(N名)」「複製する」を追加
- [x] **主催者の運営効率化 第2弾**（マイグレーション0018）: 公開予約（`publish_at`、時刻まで主催以外に非表示・RLS担保）、
      応募締切（`application_deadline`、締切後は応募不可・RLS担保）、募集テンプレート（`awase_templates`、名前付き保存→
      作成フォームで呼び出し・削除）
- [x] **主催者の運営効率化 第3弾**（マイグレーション0019）: 承認者への一斉連絡（承認済み全員へ同じ内容をDMで個別送信・
      リマインダー定型文の挿入）、キャンセル待ち受付（`accept_waitlist`、満員後も自動締切せず応募受付→応募者管理で
      「満員・キャンセル待ち」表示・繰上げ）
- [ ] **運営作業（要対応）**: Supabase SQL Editor でマイグレーション **0016〜0019** を適用（手順は [OPERATOR_CHECKLIST.md](OPERATOR_CHECKLIST.md) の
      ステップ2d〜2g）。リマインダーの自動送信が必要になったら pg_cron 運用を検討（現状は一斉連絡で手動送信）
